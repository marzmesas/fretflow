...
9. **Content pipeline** — **In progress:** **Chart QA** (validate + optional A/B compare + Practice handoff); **`midi-to-chart`** (tempo map → effective BPM, greedy fingering, `tempo-ramp` + `smoke` fixtures in CI). **Next:** importer v2 (string logic, bends), deeper QA (structural diff, golden files).
...
| 6 | `midi-to-chart` (tempo + fingering), Chart QA compare; importers / preview tool |
